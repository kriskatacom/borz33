...
final class AccountingController extends Controller
{
    ...
    public function invoicePdfs(): never { $input=Request::input();$filters=$this->accounting->filters($input);$path=$this->exports->invoicePdfPackage($filters['date_from'],$filters['date_to'],$filters['date_from'].'_'.$filters['date_to']);$this->downloadPath($path,'fakturi-'.$filters['date_from'].'_'.$filters['date_to'].'.zip'); }
    ...
    private function downloadPath(string $relative,string $name): never {$path=dirname(__DIR__,4).'/'.ltrim($relative,'/');if(!is_file($path))$this->error('Файлът не е намерен.',404);while(ob_get_level()>0)ob_end_clean();clearstatcache(true,$path);$size=filesize($path);if($size===false||$size<22)$this->error('Файлът не е валиден ZIP архив.',500);header('Content-Type: application/zip');header('Content-Disposition: attachment; filename="'.$name.'"');header('Content-Length: '.$size);header('Content-Transfer-Encoding: binary');header('X-Content-Type-Options: nosniff');readfile($path);exit;}
}
