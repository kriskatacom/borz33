...
export type SiteSettings = { logo_media_file_id: number | null; logo: MediaFile | null; vat_enabled: boolean; econt: EcontSettings };

export function getSiteSettings(token: string) {
  return apiRequest<{ settings: SiteSettings }>('/admin/settings', { token });
}

export function updateSiteSettings(token: string, body: { logo_media_file_id?: number | null; vat_enabled?: boolean; econt_environment?: 'demo' | 'production'; econt_production_username?: string; econt_production_password?: string }) {
