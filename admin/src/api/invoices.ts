...
export type Invoice = { id:number; order_id:number; order_number:string; parent_invoice_id:number|null; parent_invoice_number:string|null; type:InvoiceType; number:string|null; status:InvoiceStatus; issue_date:string|null; tax_event_date:string|null; currency:string; seller:Record<string,string|null>; buyer:Record<string,string|null>; items:Array<{name:string;sku:string;qty:number;unit_gross:number;net_total:number;tax_rate:number;tax:number;gross_total:number}>; subtotal_net:string; discount_net:string; shipping_net:string; tax_amount:string; total_gross:string; reason:string|null; has_pdf:boolean; issued_at:string|null; cancelled_at:string|null; created_at:string|null; credit_notes:Array<{id:number;number:string;status:InvoiceStatus;total_gross:string}> };
...
export function createCreditNote(token:string,id:number,reason:string,amount:string) { return apiRequest<{invoice:Invoice}>(`/admin/invoices/${id}/credit-notes`,{method:'POST',token,body:{reason,amount:amount||undefined}}); }
