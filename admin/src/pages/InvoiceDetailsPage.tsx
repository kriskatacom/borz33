...
export function InvoiceDetailsPage() {
  ...
  async function cancel() {
    setBusy(true);
    try { setInvoice((await cancelInvoice(token, invoiceId, reason)).data.invoice); toast.success('Документът е анулиран.'); }
  ...
  return <div className="page">
    ...
    <div className="grid gap-4 lg:grid-cols-[minmax(0,1.6fr)_minmax(20rem,1fr)]">
      ...
      <aside className="grid content-start gap-4">
        <section className="border border-border bg-card p-4"><h2 className="m-0 text-lg">Архивен статус</h2><p className="mt-2 font-bold">{statusLabels[invoice.status]}</p><p className="text-sm text-muted-foreground">След издаване данните и сумите не могат да се редактират.</p></section>
        ...
        {['draft', 'issued'].includes(invoice.status) ? <section className="border border-border bg-card p-4"><h2 className="m-0 mb-2 text-lg">Анулиране</h2><Button variant="outline" className="w-full" disabled={busy} onClick={() => void cancel()}><XCircle />Анулирай документа</Button></section> : null}
        {invoice.credit_notes.length ? <section className="border border-border bg-card p-4"><h2 className="m-0 mb-2 text-lg">Кредитни известия</h2>{invoice.credit_notes.map((credit) => <Link key={credit.id} className="block py-2" to={`/invoices/${credit.id}`}>№ {credit.number} · {formatMoney(credit.total_gross)}</Link>)}</section> : null}
      </aside>
    </div>
  </div>;
}
