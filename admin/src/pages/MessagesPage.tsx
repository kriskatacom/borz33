...
export function MessagesPage() {
  ...
  useEffect(() => { let cancelled = false; setBusy(true); void listMessages(token, filters).then((response) => { if (cancelled) return; setMessages(response.data.messages); setUnread(response.data.unread_count); window.dispatchEvent(new CustomEvent('admin:messages-unread-count', { detail: { count: response.data.unread_count } })); setPagination({ total: response.data.pagination.total, lastPage: response.data.pagination.last_page }); }).catch((error) => { if (!cancelled) toast.error(error instanceof ApiError ? error.message : 'Съобщенията не можаха да се заредят.'); }).finally(() => { if (!cancelled) setBusy(false); }); return () => { cancelled = true; }; }, [filters, token]);
