...
import { ArrowLeft, Mail, MapPin, Package, Phone, Save, Truck } from 'lucide-react';
...
export function OrderDetailsPage() {
  ...
  async function saveStatus() {
    ...
    setSaving(true);
    try {
      const response = await updateOrderStatus(token, order.id, status);
      setOrder(response.data.order);
      if (response.data.email_sent) toast.success(response.message || 'Статусът е обновен и клиентът е уведомен.');
  ...
  return (
    <div className="page">
      ...
      <div className="mb-4 grid gap-3 border border-border bg-card p-4 sm:grid-cols-[1fr_auto] sm:items-end">
      ...
      <div className="grid gap-4 lg:grid-cols-[minmax(0,1.6fr)_minmax(18rem,1fr)]">
        ...
        <aside className="grid content-start gap-4">
          ...
          <section className="border border-border bg-card p-4"><h2 className="m-0 mb-3 flex items-center gap-2 text-lg"><Truck className="size-5" />Доставка</h2><p className="m-0 font-bold">{deliveryLabel(order.delivery_method)}</p>{order.econt_office_code ? <p className="m-0 mt-2">Код на офис: {order.econt_office_code}</p> : null}<p className="m-0 mt-2 flex items-start gap-2"><MapPin className="mt-1 size-4 shrink-0" /><span>{order.address_line}<br />{[order.postal_code, order.city].filter(Boolean).join(' ')}, {order.country}</span></p></section>
