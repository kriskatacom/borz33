...
export function ReportsPage() {
  ...
  return <div className="page">
    ...
    <section className="report-generator" aria-labelledby="report-generator-title"><div><span><BarChart3 /></span><div><h2 id="report-generator-title">Генериране на месечен отчет</h2><p>Приходът включва платените поръчки, създадени през избрания месец, и приспада кредитните известия към тях. Повторното генериране обновява съществуващия отчет.</p></div></div><div><label htmlFor="report-period">Месец</label><MonthPicker id="report-period" value={period} max={currentPeriod()} onChange={setPeriod} /><Button type="button" disabled={generating || !period} onClick={() => void generate()}><RefreshCw className={generating ? 'animate-spin' : ''} />{generating ? 'Генериране…' : 'Генерирай отчет'}</Button></div></section>
    {busy && reports.length === 0 ? <AdminPageSkeleton variant="reports" /> : reports.length === 0 ? <div className="report-empty"><BarChart3 /><h2>Все още няма генерирани отчети</h2><p>Изберете месец и създайте първия отчет.</p></div> : <div className="report-list">{reports.map((report) => <article className="report-card" key={report.id}><header><div><p>Месечен отчет</p><h2>{monthLabel(report)}</h2><small>Генериран от {report.generated_by} · {formatDateTime(report.generated_at)}</small></div><strong>{formatMoney(report.recognized_revenue)}</strong></header><div className="report-metrics"><div><span><ShoppingBag />Признат приход</span><strong>{formatMoney(report.recognized_revenue)}</strong></div><div><span><PackageCheck />Платени поръчки</span><strong>{report.paid_orders_count}</strong></div><div><span><CalendarDays />Средна поръчка</span><strong>{formatMoney(report.average_order_value)}</strong></div><div><span><Truck />Приход от доставка</span><strong>{formatMoney(report.shipping_revenue)}</strong></div></div><details><summary>Пълен отчет</summary><div className="report-details"><section><h3>Обобщение</h3><dl><div><dt>Всички поръчки</dt><dd>{report.orders_count}</dd></div><div><dt>Доставени поръчки</dt><dd>{report.delivered_orders_count}</dd></div><div><dt>Отказани поръчки</dt><dd>{report.cancelled_orders_count}</dd></div><div><dt>Оборот преди кредитните известия</dt><dd>{formatMoney(report.gross_turnover)}</dd></div><div><dt>Кредитни известия</dt><dd>{report.credit_notes_count} · −{formatMoney(report.credit_notes_amount)}</dd></div><div><dt>Приход от продукти</dt><dd>{formatMoney(report.product_revenue)}</dd></div><div><dt>Продадени артикули</dt><dd>{report.items_sold}</dd></div></dl></section><section><h3>Поръчки по статус</h3><dl>{Object.entries(report.status_breakdown).map(([status, metric]) => <div key={status}><dt>{statusLabels[status] ?? status} ({metric.count})</dt><dd>{formatMoney(metric.total)}</dd></div>)}</dl></section><section className="report-top-products"><h3>Водещи продукти</h3>{report.top_products.length === 0 ? <p>Няма продукти в платени поръчки за периода.</p> : <ol>{report.top_products.map((product, index) => <li key={`${product.name}-${product.sku}-${index}`}><span><strong>{product.name}</strong><small>{product.sku || 'Без SKU'} · {product.qty} бр.</small></span><b>{formatMoney(product.revenue)}</b></li>)}</ol>}</section></div></details></article>)}</div>}
  </div>;
}
