...
<body class="min-h-full bg-canvas text-ink antialiased">
    ...
    >
        ...
        <main id="content" class="mx-auto <?= str_starts_with($currentPath, '/account') ? 'w-[min(1440px,calc(100%-2rem))]' : 'w-[min(1320px,calc(100%-2rem))]' ?> flex-1 <?= $compactMainBottom ? 'pb-3' : 'pb-14' ?> <?= $flushMainTop ? 'pt-0' : ($currentPath === '/' ? 'pt-3' : 'pt-7') ?>">
            <?= $content ?>
        </main>
